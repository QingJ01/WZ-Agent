...
如果项目根目录存在 `scrcpy/` 本地工具包，运行时会优先使用其中的 `adb.exe`，并自动把 `scrcpy/` 加入 `PATH`。GUI 默认使用 `scrcpy` 强制帧源，Android 真机输入默认走 scrcpy control socket；如需兼容模式，可在 GUI 中切换为 `auto` 或 `ADB 截图`。该目录用于本机运行，不会提交到仓库。
